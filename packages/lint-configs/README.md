...
```
ok     .sarj-standards.toml  --  version 0.20.0
drift  .github/workflows/ci.yml: sarj-python-lint==0.12.2  --  installed sarj-python-lint is 0.33.0
drift  pyproject.toml: sarj-python-lint==0.25.0  --  installed sarj-python-lint is 0.33.0
ok     pyproject.toml: sarj-lint-configs==0.20.0  --  matches the installed wheel
drift  package.json: @sarj/eslint-plugin@2.16.0  --  the bundled eslint.strict.mjs is tested against 5.0.0
```
...
namespace (`python-v0.33.0` for `sarj-lint-configs` 0.20.0) that nobody should
...
| --- | --- |
| `@sarj/eslint-plugin` | Every custom rule the config names has to exist. `prefer-zod-infer` arrived in 4.1.0 and `prefer-zod-enum` in 2.17.0; naming a rule the installed plugin lacks is "Definition for rule was not found", once per file. |
...
package and no Python. It is flat-config shaped as of `@sarj/eslint-plugin@5.0.0`;
before that both presets declared `plugins` in eslintrc array form and ESLint 9
