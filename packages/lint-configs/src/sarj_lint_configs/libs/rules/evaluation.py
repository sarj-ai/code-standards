...
from sarj_lint_configs.libs.corpus import CorpusSnapshot
...
class EvaluationEvidence:
    ...
    def __post_init__(self) -> None:
        if not self.corpora or not self.sample_method.strip():
            msg = "evaluation evidence requires verified corpora and a sampling method"
            raise ValueError(msg)
        if self.elapsed < timedelta() or self.baseline < timedelta():
            msg = "evaluation timings must be non-negative"
            raise ValueError(msg)
    ...
    *,
    ...
    for case in cases:
        ...
        if wrong:
            ...
            raise ValueError(msg)
        outcomes.append(
            CaseEvaluation(case.report_id, case.report_path, case.expected, _report_findings(case, findings))
        )
