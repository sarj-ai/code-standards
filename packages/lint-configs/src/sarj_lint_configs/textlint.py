...
from .manifest import as_table, list_field, table_field
...
class Finding:
    ...
    def render(self) -> str:
        ...
        return f"{self.path}:{self.line}:1: {self.code} {self.message}"
...
def _durable_patterns(root: Path) -> tuple[str, ...]:
    ...
    try:
        ...
        return _DURABLE_MARKDOWN
    artifacts = table_field(as_table(parsed), "artifacts")
    configured = list_field(artifacts, "durable")
