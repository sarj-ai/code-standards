...
from sarj_lint_configs import library_policy
...
def test_cli_enforces_only_the_application_profile(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    (tmp_path / "package.json").write_text('{"dependencies":{"axios":"1"}}', encoding="utf-8")

    assert cli.main(["library-policy", "--dest", str(tmp_path)]) == 0
    assert "standard profile" in capsys.readouterr().out
    assert cli.main(["library-policy", "--profile", "application", "--dest", str(tmp_path)]) == 1
    output = capsys.readouterr().out
    assert "LIB101" in output
    assert "Ky" in output


def test_cli_reports_manifest_parse_errors_as_infrastructure_failures(
    tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    (tmp_path / "package.json").write_text("{", encoding="utf-8")

    assert cli.main(["library-policy", "--profile", "application", "--dest", str(tmp_path)]) == 2
