...
ExampleOutcome = ExpectedOutcome
Severity = DefaultLevel
...
class ExampleFile:
    ...
    @classmethod
    def python(cls, path: str | PurePosixPath, source: str) -> Self:
        return cls(PurePosixPath(path), source)

    @classmethod
    def sql(cls, path: str | PurePosixPath, source: str) -> Self:
        return cls(PurePosixPath(path), source)

    @classmethod
    def iac(cls, path: str | PurePosixPath, source: str) -> Self:
        return cls(PurePosixPath(path), source)
...
class RuleSpec(RuleDocumentation):
    engine: RuleEngine
    rule_id: RuleId
    code: str | None
    ...
    def __post_init__(self) -> None:
        RuleDocumentation.__post_init__(self)
        ...
        if self.options_schema is not None:
            try:
                parsed_schema: object = json.loads(self.options_schema)  # pyright: ignore[reportAny] -- narrow the standard-library JSON boundary below.
    ...
    def _validate_identity(self) -> None:
        ...
        if not self.languages:
            ...
            raise ValueError(msg)
        if self.rule_id in self.aliases:
            msg = "rule aliases must be historical IDs, not the live rule ID"
            raise ValueError(msg)
...
@dataclass(frozen=True, slots=True, kw_only=True)
class NativeRuleSpec(RuleDocumentation):
    engine: str
    rule_id: str
    code: str
