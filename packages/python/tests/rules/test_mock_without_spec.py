...
def test_plain_helper_parameter_named_mocker_is_not_assumed_to_be_pytest_mock() -> None:
    src = """
def build_double(mocker):
    return mocker.Mock()

def test_thing(tool):
    assert build_double(tool)
"""

    assert _check(src) == []


def test_pytest_fixture_receiving_mocker_is_analyzed() -> None:
    src = """
import pytest
...
def test_unrelated_local_shadow_does_not_hide_module_import() -> None:
    src = """
from unittest import mock
from unittest.mock import Mock

def helper(mock, Mock):
    return mock, Mock
...
@pytest.mark.parametrize("factory", ["Mock", "MagicMock", "AsyncMock", "NonCallableMock", "NonCallableMagicMock"])
@pytest.mark.parametrize("kwarg", ["autospec=True", "new=replacement", "new_callable=Factory"])
def test_patch_only_keywords_do_not_constrain_mock_constructors(factory: str, kwarg: str) -> None:
    src = f"""
from unittest import mock

def test_thing():
    double = mock.{factory}({kwarg})
    double.missing_collaborator_method()
"""
...
@pytest.mark.parametrize("create", ["True", "dynamic_create"])
def test_patch_of_a_possibly_missing_attribute_is_exempt(create: str) -> None:
    src = f"""
from unittest.mock import patch

def test_thing():
    with patch("app.future_hook", create={create}):
        pass
"""
...
def test_patch_create_false_still_requires_a_contract() -> None:
    src = """
from unittest.mock import patch

def test_thing():
    with patch("app.send", create=False):
        pass
"""
...
def test_thing():
    with mock.patch("app.send", new={replacement}):
        pass
"""
...
def test_positional_patch_default_still_generates_an_unrestricted_mock() -> None:
    src = """
from unittest.mock import DEFAULT, patch

def test_thing():
    with patch("app.send", DEFAULT):
        pass
"""
...
def test_new_callable_none_still_uses_the_default_mock_factory() -> None:
    src = """
from unittest.mock import patch

def test_thing():
    with patch("app.send", new_callable=None):
        pass
"""
...
def test_generated_test_is_ignored() -> None:
    assert _check(f"# generated by protoc\n{_BARE_MOCK}") == []
...
def test_mock_add_spec_before_domain_use_constrains_the_double() -> None:
    src = """
from unittest.mock import Mock

def test_client():
    client = Mock()
    client.mock_add_spec(Client, spec_set=True)
    client.send()
"""
...
def test_mock_add_spec_after_domain_use_does_not_hide_the_unspecced_access() -> None:
    src = """
from unittest.mock import Mock
