...
def test_header_keywords_without_body_are_not_code(body: str):
...
@pytest.mark.parametrize(
    "body", ["_a = %sent%", "config[...] = default", "handler = <FunctionBody>", "opt = value[opt]"]
)
def test_pseudocode_placeholder_markers_are_not_flagged(body: str):
    assert _standalone(body) == [], body
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "in a real app we would fetch the account",
    ],
)
def test_explicit_editorial_placeholders_are_cruft(body: str) -> None:
    diags = _standalone(body)
    assert len(diags) == 1
...
def test_editorial_placeholder_inside_a_string_is_not_a_comment() -> None:
    assert _check('message = "implementation omitted"\n') == []
...
@pytest.mark.parametrize("body", ["debug(v)", "insert_asserts(v)", "my_insert_assert(v)"])
def test_other_commented_out_dev_tool_calls_still_fire(body: str):
...
def test_license_header_rules_are_not_section_banners():
    assert _check(f"{_LICENSE_HEADER}import os\n") == []
