...
def test_two_sentence_warning_does_not_fail(tmp_path: Path) -> None:
    target = tmp_path / "example.py"
    target.write_text("# First fact. Second fact.\nvalue = 1\n")

    assert main(["check", "--rule", "prefer-single-sentence-comment", str(target)]) == 0
...
def test_warnings_are_visible_even_when_errors_are_baselined() -> None:
    ...
    warning = Diagnostic(path, 1, 1, "SARJ090", "shorten", Severity.WARNING)
