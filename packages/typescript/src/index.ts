...
const recommendedRules = {
  ...
  // Shipped in the plugin and wired into the shared `eslint.strict.mjs`,
  // but absent from BOTH presets until 5.0.0 — so anyone reaching for
  // `configs.strict` instead of the shared config silently never ran it.
  // Caught by the new "strict wires every rule the plugin ships" test.
...
const strictRules = {
  ...
  // See the `recommended` table: wired into neither preset before 5.0.0.
 ...
 *
 * Until 5.0.0 both declared `plugins: ["@sarj"]` — eslintrc syntax — and ESLint
 ...
 * path, threw for anyone who tried it. What DID work was copying
 * `eslint.strict.mjs` and editing the copy — exactly the vendoring these presets
 * exist to prevent, and exactly the drift measured across consumers. `plugins`
 * is now the object form flat config wants, so both presets go straight into an
 * `eslint.config.mjs` array (no spread):
 *
 *     import sarj from "@sarj/eslint-plugin";
 *     export default [sarj.configs.strict];
 *
 * Neither preset sets `files` or a parser. They carry rules and nothing else, so
 * they compose with whatever a repo already has rather than narrowing it.
 */
const plugin = {
  meta,
  rules,
...
export { recommendedRules, rules, strictRules };
