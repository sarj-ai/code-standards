...
def _root_readme(
    ...
    sections = [
        ...
        (
            ...
            "```bash\nmake setup\nmake verify\n```"
            "\n\nEvery proposed rule, whether custom or upstream, must include executable positive and "
            "negative behavioral unit tests plus public bad and good examples. Configuration-only "
            "assertions do not count as behavioral coverage. Once a custom rule and its tests are "
            "registered, stage it as a warning and validate it locally:\n\n"
            "```bash\n"
