...
_GENERATED_CONFIG_OWNERSHIP_RE = re.compile(
    r"^(?:@generated\b|auto(?:-|matically )?generated(?:[.!]?|\s+file\b.*|\s+by\b.*)$|"
    r"code generated by\b|generated by\b|"
    r"(?:this\s+)?file\s+(?:is|was|has been)\s+(?:auto(?:-|matically )?)?generated\b)",
    re.IGNORECASE,
)
_GENERATED_CONFIG_HANDS_OFF_RE = re.compile(r"\bdo not (?:hand[- ]?)?edit\b", re.IGNORECASE)
...
REGISTRY: Final[Mapping[str, RuleMeta]] = MappingProxyType(
    {
        "config-comment-wall": RuleMeta(
            ...
            examples=(
                ...
                _public_example(
                    example_id="config-rationale",
                    title="Rationales keep a mixed comment cluster useful",
                    outcome=ExpectedOutcome.NO_MATCH,
                    path="workflow.yml",
                    source="# Keep one worker because uploads race.\nconcurrency: 1\n"
            ...
            limitations=(
                ...
                "Files with a leading generated-ownership marker and an explicit do-not-edit instruction are excluded.",
                "Only clusters of at least four attached same-indent comments within one configuration owner are compared.",
                "Clusters stop at blank lines, YAML document markers, TOML table headers, or dedentation out of the current owner.",
                "Substantive action verbs must also appear in the adjacent entry; set, define, and configure are treated as structural narration.",
            ),
        ),
...
def _has_generated_config_header(path: Path, lines: list[str]) -> bool:
    ...
    for line in lines[:12]:
        ...
        if parsed is None:
            break
        if parsed.body:
            comments.append(parsed.body)
    header = "\n".join(comments)
    return (
        any(_GENERATED_CONFIG_OWNERSHIP_RE.match(comment) for comment in comments)
        and _GENERATED_CONFIG_HANDS_OFF_RE.search(header) is not None
    )
    ...
    *,
    ...
    for entry in attached:
        if (
            ...
            and not _comment_wall_boundary(lines, groups[-1][-1][0], entry[0], indent=entry[1], path=path)
        ):
...
def _comment_wall_boundary(lines: list[str], previous: int, current: int, *, indent: int, path: Path) -> bool:
    suffix = path.suffix.lower()
    for index in range(previous, current - 1):
        line = lines[index]
        stripped = line.strip()
        if not stripped or (suffix in {".yaml", ".yml"} and stripped in {"---", "..."}):
            return True
        if suffix == ".toml" and re.fullmatch(r"\[\[?.+?\]\]?", stripped):
            return True
        if _standalone_comment(path, line) is None and len(line) - len(line.lstrip()) < indent:
            return True
    return False
...
def _weak_narration(body: str, statement: str) -> bool:
    if len(body.split()) > _WALL_MAX_WORDS or not _NARRATION_RE.match(body):
        return False
    words = [_normalize_word(word) for word in _words(body)]
    action, *remainder = words
    code = {_normalize_word(word) for word in _words(statement)}
    if action not in {"configure", "define", "set"} and action not in code:
        return False
    content = [word for word in remainder if word not in _STOPWORDS]
    if not content:
        return False
