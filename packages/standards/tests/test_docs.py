...
def test_sync_is_deterministic_and_check_then_passes(tmp_path: Path) -> None:
    ...
    assert "executable positive and negative behavioral unit tests" in rendered
    assert "public bad and good examples" in rendered
    assert "Configuration-only assertions do not count" in rendered
