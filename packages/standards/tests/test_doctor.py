...
def test_doctor_rejects_non_string_manifest_destinations_without_a_traceback(
    ...
    assert cli.main(["--root", str(tmp_path), "doctor"]) == 2
...
def test_doctor_reports_a_legacy_bootstrap_launcher(tmp_path: Path) -> None:
    ...
    findings = [finding for finding in doctor.diagnose(tmp_path) if finding.id == "doctor.launcher.legacy"]

    assert len(findings) == 1
    assert findings[0].level is doctor.Level.DRIFT
    assert findings[0].where == ".github/workflows/standards.yml"
...
def test_doctor_falls_back_to_bounded_filesystem_walk_when_git_times_out(
    ...
    (tmp_path / "requirements.txt").write_text("sarj-standards==0.0.1\n", encoding="utf-8")
