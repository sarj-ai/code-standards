...
Run the shared **[stack-detection](./stack-detection.md)** pass first. **For this rule, detect the DB/runtime atomicity primitives:** Postgres + psycopg (the flagship product, the ML services repo) ⇒ interactive transactions (`async with conn.transaction()`), `INSERT ... ON CONFLICT`, and `jsonb_set` apply. Cloudflare D1 (the internal Workers app) ⇒ there are **no interactive transactions** (use `env.DB.batch()` or a Durable Object for cross-statement atomicity), use `INSERT ... ON CONFLICT` / `INSERT OR IGNORE` and `json_set()` (not `jsonb_set`), and queue-consumer idempotency comes from a message-derived key + read-before-write. Do not assume an ORM (`db.query.findFirst`) — both Python and TS use raw SQL. Then add the skill-specific items:
