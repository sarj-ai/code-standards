...
Run the shared **[stack-detection](./stack-detection.md)** pass first. **Critically for this rule: detect tenancy** — grep the schema/migrations for `organization_id`/`tenant_id`/`workspace_id`. If present (e.g. the flagship product), the IDOR/ownership-scoping checks below apply in full. If absent (e.g. the single-tenant internal Workers app), **skip the multi-tenant IDOR concern** and instead scope on whatever ownership column exists (token, `user_id`). Also detect the SQL placeholder style (`?` for D1/SQLite vs `%s` for psycopg). Then add the skill-specific items:
