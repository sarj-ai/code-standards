...
Run the shared **[stack-detection](./stack-detection.md)** pass first. **For this rule, detect the config mechanism per runtime:** Cloudflare Workers (the internal Workers app) have **no `process.env`** — config arrives as bindings via `c.env`, so a `process.env` grep finds nothing real; Python services use Pydantic Settings (and `os.environ` is banned via `TID251` in the flagship product). Do **not** flag the deliberate `.optional().default('')` fail-safe env defaults / `BooleanFlagSchema` transforms as missing-config. Only cite lint rules the package actually runs. Then, add the following skill-specific discovery steps:
