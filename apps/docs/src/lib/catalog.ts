...
export type Language =
  "config" | "iac" | "markdown" | "python" | "shell" | "sql" | "typescript";
...
const schemaPath = resolve(
  process.cwd(),
  "../../packages/standards/src/sarj_standards/schemas/rule-catalog.v1.schema.json",
);
...
function validateCatalog(value: unknown): asserts value is Catalog {
  if (
    ...
    throw new TypeError(
      "rule catalog must contain schemaVersion 1 and a rules array.",
    );
  }
  ...
  for (const [index, rule] of value.rules.entries()) {
    ...
    if (keys.has(rule.key))
      throw new TypeError(`rule catalog contains duplicate key ${rule.key}.`);
    keys.add(rule.key);
    rules.push(rule);
  }
  ...
  for (const rule of rules) {
    for (const alias of rule.aliases) {
      ...
      if (activeIds.has(aliasKey))
        throw new TypeError(`rule alias ${aliasKey} shadows an active rule.`);
      if (aliases.has(aliasKey))
        throw new TypeError(`rule alias ${aliasKey} has multiple targets.`);
...
function validateRule(value: unknown, index: number): asserts value is Rule {
  if (!isRecord(value))
    throw new TypeError(
      `rule catalog rules[${String(index)}] must be an object.`,
    );
  ...
  if (!ENGINES.includes(engine as Engine))
    throw new TypeError(`${context}.engine is unsupported: ${engine}.`);
  if (key !== `${engine}:${id}`)
    throw new TypeError(`${context}.key must equal ${engine}:${id}.`);
  for (const field of [
    "summary",
    "rationale",
    "remediation",
  ...
  if (!Array.isArray(value.examples))
    throw new TypeError(`${context}.examples must be an array.`);
...
function validateExample(
  value: unknown,
  context: string,
): asserts value is RuleExample {
  if (!isRecord(value)) throw new TypeError(`${context} must be an object.`);
  for (const field of ["id", "scenarioId", "title", "focusPath"])
    requireString(value, field, context);
  ...
  if (
    ...
    throw new TypeError(
      `${context}.expectedCount must be a non-negative integer.`,
    );
  }
  ...
  if ((value.files as unknown[]).length === 0)
    throw new TypeError(`${context}.files must not be empty.`);
}

function validateExampleFile(
  value: unknown,
  context: string,
): asserts value is ExampleFile {
  if (!isRecord(value)) throw new TypeError(`${context} must be an object.`);
...
const rawCatalog = await readGeneratedJson(
  catalogPath,
...
export const catalogSchema = await readGeneratedJson(
  schemaPath,
  "Rule catalog schema",
);
